...
function start_chart_render_process() {
    $("#chart_ld").fadeIn();
    var get_data = { "ticker": $('#autocomplete').val(), "expiry_date": $('#expiry').val() };
    $.ajax({
        ...
        success: function(data){
            $("#chart_ld").fadeOut();
            $("#warning").fadeOut();
            render_chart_from_api(data, "openinterest_chart");
        },
        error: function (err) {
        $("#chart_ld").fadeOut();
        $("#warning").fadeIn();    
        ...
        processData: false,
        type: 'POST',
        url: 'http://srirajk.herokuapp.com/openinterest'
    });
}
