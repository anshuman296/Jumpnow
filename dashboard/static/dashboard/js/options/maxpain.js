...
function start_chart_render_process() {
    $("#chart_ld").fadeIn();
    var prices = $("#strike").val();
    var intraday_ind = $('input[name="multiday"]:checked').val();
    if (intraday_ind == 'true') {
        intraday_ind = true;
    }
    else {
        intraday_ind = false;
    }
    var get_data = { "ticker": $('#autocomplete').val(), "expiry_date": $('#expiry').val(), "strikes_count": parseInt($("#strikes_count").val())};
    $.ajax({
        ...
        success: function(data){
            $("#chart_ld").fadeOut();
            $("#warning").fadeOut();
            render_chart_from_api(data, "maxpain_chart");
        },
        error: function (err) {
        $("#chart_ld").fadeOut();
        $("#warning").fadeIn();    
        ...
        processData: false,
        type: 'POST',
        url: 'http://srirajk.herokuapp.com/maxpain'
    });
}



$("#maxpain-form").validate({
